...
export function registerTargetsRoutes(app: FastifyInstance, config: Config): void {
  ...
  app.post("/api/targets/upload", async (request, reply) => {
    ...
    try {
      ...
      await uploadTargetsBlob(config.targets.storageAccount, rawBuffer, outcome.data, uploadedBy, uploadedAt, { unconfirmed: [] });
    } catch (err) {
  ...
  app.post("/api/targets/import-written", async (request, reply) => {
    ...
    try {
      ...
      await uploadTargetsBlob(config.targets.storageAccount, mortgageBuffer, merged, uploadedBy, uploadedAt, { note, unconfirmed: [...TARGETED_KPI_KEYS] });
    } catch (err) {
      logger.error("Targets blob write failed", { err: String(err) });
      return reply.code(502).send({ error: "Import validated but could not be saved — please try again." });
    }
  ...
  app.post("/api/targets/set-leads", async (request, reply) => {
    ...
    // Leads is now Capricorn's; whatever else was already unconfirmed stays that way. Read from live
    // provenance rather than assuming, so this can't quietly mark Applications as confirmed too.
    const stillUnconfirmed = (getTargetsProvenance().unconfirmed ?? [...TARGETED_KPI_KEYS]).filter((k) => k !== "leads");
    ...
    activateTargets(merged, uploadedBy, uploadedAt, note, stillUnconfirmed);
